    ...
    <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '24px' }}>
      ...
      {scanDetail?.identity_warnings && scanDetail.identity_warnings.length > 0 && (
        <div style={{ padding: '16px', background: 'rgba(233, 185, 73, 0.12)', border: '2px solid var(--accent-review)', borderRadius: '10px', marginBottom: '20px' }}>
          ...
          {scanDetail.identity_warnings.map((w: any, idx: number) => (
                                      ...
                                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                        <span style={{ fontWeight: 700, color: 'var(--text-secondary)' }}>
                                          {(() => {
                                            ...
                                            return ev?.source === 'YOLO' || ev?.source === 'HEURISTIC' ? 'ML Detection' : 'Extracted Evidence';
                                          })()}
                                        </span>
                                        {(() => {
                                          const ev = expandedEvidence.find(e => e.field_name === finding.field);
                                        ...
                                        return (
                                          <>
                                            ...
                                            {(ev.model_name || ev.source === 'YOLO' || ev.source === 'HEURISTIC') && (
                                              <div style={{ marginTop: '8px', padding: '6px', background: 'rgba(59, 130, 246, 0.08)', borderRadius: '4px', borderLeft: '3px solid #3b82f6', fontSize: '0.7rem' }}>
                                                <span style={{ color: '#3b82f6', fontWeight: 600 }}>Evidence generated by: </span>
                                                <span style={{ color: 'var(--text-primary)' }}>{ev.model_name || 'YOLOv8 Panel Detector'} {ev.model_version ? `v${ev.model_version}` : 'v1'}</span>
                                              </div>
                                            )}
